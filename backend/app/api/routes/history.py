...
async def compare_years(
    ...
    try:
        ...
        if len(data.metrics) < 2:
            raise HTTPException(status_code=400, detail="Data unavailable for both years")
        ...
        return {
            ...
            "year1": year1,
            "year2": year2,
            "ndvi_diff": m2.ndvi - m1.ndvi,
            "water_diff_ha": m2.waterAreaHa - m1.waterAreaHa,
            "green_cover_diff_percent": m2.greenCoverPercent - m1.greenCoverPercent
        }
    except VillageNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
