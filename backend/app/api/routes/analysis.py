...
async def get_environmental_analysis(
    ...
    try:
        ...
        # Mock area for now - in production would compute from boundary geometry
        village_area_km2 = 50.0 
        
        # Interpretations
        ndvi_analysis = interpret_ndvi(metrics.ndvi)
        water_analysis = interpret_water_stress(metrics.ndwi, metrics.waterAreaHa, village_area_km2)
        
        terrain_raw = raw_metrics.get("terrain") or {}
        lc_raw = raw_metrics.get("land_cover") or {}
        flood_analysis = assess_flood_risk(terrain_raw, lc_raw, metrics.rainfall)
